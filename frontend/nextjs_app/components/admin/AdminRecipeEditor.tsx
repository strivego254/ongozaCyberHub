...
export function AdminRecipeEditor({
  ...
  useEffect(() => {
    if (recipe) {
      setFormData({
        ...
        track_code: recipe.track_code || 'defender',
  ...
  const validateForm = () => {
    ...
    if (!formData.skill_code?.trim()) newErrors.skill_code = 'Skill code is required';
    if (!formData.track_code) newErrors.track_code = 'Track is required';
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateField('skill_code', e.target.value)}
                        className={`w-full px-3 py-2 bg-slate-800/50 border rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 ${
                          errors.skill_code ? 'border-red-500' : 'border-slate-700/50'
                        }`}
                        placeholder="log_parsing"
                      />
                      {errors.skill_code && <p className="text-red-400 text-sm mt-1">{errors.skill_code}</p>}
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-slate-300 mb-2">Track *</label>
                      <select
                        value={formData.track_code || 'defender'}
                        onChange={(e) => updateField('track_code', e.target.value)}
